...
class InAppPurchaseAndroidPlatformAddition
  ...
  Future<QueryPurchaseDetailsResponse> queryPastPurchases({
    ...
    // queryPurchases 的外层状态为历史兼容值；错误聚合必须使用原生返回的真实状态。
    final Set<String> errorCodeSet = responses
        .where(
          (PurchasesResultWrapper response) =>
              response.billingResult.responseCode != BillingResponse.ok,
        )
        .map(
          (PurchasesResultWrapper response) => response.billingResult.responseCode.toString(),
        )
        .toSet();
